/**
  ...
  * diWave_stributed under the License is diWave_stributed on an "AS IS" BASIS, 
...
uint8_t Downturn_audio_global = 6; // 0-8, 0 no, 8 max - mute
...
extern volatile uint32_t tmp_nvic;
...
void WavePlayer_Init(void)
{
  ...
  NVIC_Init(&NVIC_InitStructure);
	...
	/* TIM6 TRGO selection */
        ...
        //TIM_SetAutoreload(TIM6, 2176);    // Read the Sample Rate 22.05KHz = 48MHz / 2176 
...
/**
  ...
  *         retrieves the audio format in WAVE_Format Wave_structure and returns a zero
...
void Wave_Player_Play(char* FileName)
{
  ...
  TIM_SetAutoreload(TIM6, TIM6ARRValue);
  //TIM_SetAutoreload(TIM6, WAVE_Format.SampleRate);    // Read the Sample Rate 22.05KHz = 48MHz / 2176 
...
/**
  * @brief  Interupt handler  DMA1 channel 3, DMA1_IT_TC3, wave playing
  * @param  None
  * @retval None
  */
void Wave_Player_Play_Handler(void)
{
  ...
  if(!WaveDataLength)
  {
    ...
    if(buffer_active == 0)
    {
      ...
      DMA1_Channel1->CPAR = DAC_DHR8R1_Address;
