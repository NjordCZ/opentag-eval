...
extern uint8_t* SIRC_devices[];
extern uint8_t* SIRC_Commands[];
...
void SIRC12_Encode_SignalGenerate(uint32_t SIRC12_FramePulseWidthFormat[])
{
  ...
  if((SIRC12_Send_Operation_Ready_f != RESET) && (SIRC12_BitsSent_Counter <= ( SIRC12_COUNTER_LIMIT )))
  {
    ...
    if (SIRC12_BitsSent_Counter < SIRC12_CodedFrameLastWordLength) 
    {
      ...
      bitmsg = (uint8_t)((SIRC12_FramePulseWidthFormat[0] >> SIRC12_BitsSent_Counter)& 1);
    }
    else
    {
      /*Read coded frame bits from the table*/
      bitmsg = (uint8_t)((SIRC12_FramePulseWidthFormat[((SIRC12_BitsSent_Counter-SIRC12_CodedFrameLastWordLength)/32)+1] >> SIRC12_BitsSent_Counter-SIRC12_CodedFrameLastWordLength)& 1);
    }
    /* Generate signal */
    if (bitmsg == SET)
    {
  ...
  else
  {
    ...
    {
      SIRC12_FramePulseWidthFormat[tablecounter] = 0x0;
    }
    TIM_ForcedOC1Config(TIM16, TIM_ForcedAction_InActive);
  }
}
